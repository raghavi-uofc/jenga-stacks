...
// In-memory team store (demo only)
const TEAM_STORE = [];
...
// --- Team import/list endpoints (demo) ---
app.get("/teams", (req, res) => {
  res.json({ items: TEAM_STORE });
});
...
app.post("/teams/import", (req, res) => {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    return res.status(400).json({ error: "Expected an array of team members" });
  }

  let inserted = 0;
  let updated = 0;
  const byEmail = new Map(TEAM_STORE.map((m) => [String(m.email).toLowerCase(), m]));
  ...
  for (const raw of items) {
    ...
    if (byEmail.has(key)) {
      ...
      TEAM_STORE.push(member);
      byEmail.set(key, member);
      inserted += 1;
    }
  }

  return res.json({ ok: true, inserted, updated, total: TEAM_STORE.length });
});
